...
def fetch_data():
    ...
    return customer_data.to_json(orient='records')
...
def apply_kprototypes_route():
    customer_data = fetch_customer_data()
    preprocessed_data = preprocess_data(customer_data)
    clusters, kproto = apply_kprototypes(preprocessed_data)
    return jsonify({"clusters": clusters.tolist(), "kproto": str(kproto)})
...
def visualize():
    ...
    preprocessed_data = preprocess_data(customer_data)
    clusters, kproto = apply_kprototypes(preprocessed_data)
    pairplot_filename, barplot_filename, scatterplot_filenames = visualize_clusters(preprocessed_data, clusters, kproto)
    return jsonify({
        "pairplot_filename": pairplot_filename,
        "barplot_filename": barplot_filename,
        "scatterplot_filenames": scatterplot_filenames
    })
