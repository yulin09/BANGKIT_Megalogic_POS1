...
def plot_sales(test_data, y_pred):
    ...
    plt.xlabel('Date')
    plt.ylabel('Sales')
    plt.savefig('static/images/sales_plot.png')
...
def future_sales_next_month_route():
    ...
    merged_df = preprocess_data(orders_df, products_df)
    sales_data = aggregate_sales_by_date(merged_df)
    
    n_past = 30
    n_future = 30
    model, scaler = train_lstm_model(sales_data, n_past, n_future)
    
    future_data_monthly, next_month_prediction, next_trimester_prediction = future_sales_predictions(
        model, scaler, sales_data, n_past, n_future
    )
    return jsonify({'next_month_prediction': next_month_prediction})
...
def future_sales_trimester_route():
    ...
    return jsonify({'next_trimester_prediction': next_trimester_prediction})
